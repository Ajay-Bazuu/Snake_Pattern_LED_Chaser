...
#define SOUND_SENSOR_PIN 34  // Change this to the appropriate pin for your sound sensor
...
void loop() {
  int soundValueMax = 2200;
  int soundThreshold = 1900;
  int soundValue = analogRead(SOUND_SENSOR_PIN);
  Serial.println(soundValue);

  // Adjust the threshold based on your sensor characteristics
  if (soundValue > 1850 && soundValue < 1950) {
    Serial.println("Dead Zone");
    soundValue = soundThreshold;
  } else if (soundValue < 1700 || soundValue > 2150) {
    soundValue = soundValueMax;
  }

  if (soundValue > soundThreshold) {
    int numLEDsToGlow = map(soundValue, soundThreshold, soundValueMax, 3, 6);
